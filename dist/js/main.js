...
async function updateTransactionPool(table) {
      ...
      while (ctn) {
        ...
        if (____dataJSON.type == 2) { _Type = "Web3 Transaction"; var tx = new ethereumjs.Tx(____dataJSON.rawTx); _To = '0x'+tx.to.toString('hex'); _Amount = parseInt(tx.value.toString('hex') || '0', 16) / (10**18); URL_link = "./Web3_Transaction.html?hash="+____hash; }
